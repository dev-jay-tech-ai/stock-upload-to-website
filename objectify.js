// [ title(0), price(1), saleprice(2), color(3), sizeOpt(4), content(5), imgLinks(6), brand(7), origin(8), material(9), formSize(10), barcode(11), productCode(12) ]
// const arr = [
...
module.exports = (arr) => {
  ...
  colOpt.forEach((color,idx) => { // 재고가 있는 컬러만큼 반복
    let sizeOpt = [...new Set(arr[4][idx].split(','))].slice();
  ...
  options[1].values = [...new Set(arr[4].join().split(','))];
