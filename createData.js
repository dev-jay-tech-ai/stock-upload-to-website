...
let arr = [];
results.map((result) => { // [ subject(0), price(1), saleprice(2), color(3), size(4), content(5), imgLinks(6), brand(7), origin(8), material(9), formSize(10) ]
  ...
  arr.push([ result[0], currency(result[1]), currency(result[2]), result[3], result[4], 'content', result[6], brand, origin, material, formSize ]);
});
